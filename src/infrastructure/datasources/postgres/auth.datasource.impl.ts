...
type ComparePasswordFuncion = (password: string, hashedPassword: string) => boolean;
...
export class AuthDataSourceImpl implements AuthDatasource {
	...
	async login(loginUserDto: LoginUserDto): Promise<User> {
		...
		if (res.rows.length === 0) throw CustomError.badRequest('Problem with your credentials.');
		...
		if (!this.comparePassword(password, hashedPassword)) throw CustomError.badRequest('Problem with your credentials.');
	...
	async register(registerUserDto: RegisterUserDto): Promise<User> {
		...
		const createUserQuery = 'INSERT INTO "user" (email, name, password) VALUES ($1, $2, $3) RETURNING *;';
