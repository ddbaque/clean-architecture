...
export class UserDataSourceImpl implements UserDatasource {
  ...
  async getUser(getUserDto: GetUserDto): Promise<User> {
    ...
    if (resGetUserById.rowCount === 0)
      throw new CustomError(404, "User not found.", ErrorCode.NOT_FOUND);

    return Promise.resolve(
      UserMapper.userEntityFromObject(resGetUserById.rows[0]),
    );
  }
  async getAllUsers(): Promise<Array<User>> {
    ...
    return Promise.resolve(
      UserMapper.userArrayEntityFromObject(resGetAllUsers.rows),
    );
  }
}
